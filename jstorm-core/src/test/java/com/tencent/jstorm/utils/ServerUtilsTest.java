...
public class ServerUtilsTest {
  ...
  public static String getTestFilePath(String fileName) {
    URL url = ServerUtilsTest.class.getClass().getResource("/" + fileName);
    return url.getPath();
  }
  ...
  public void testParseList() {
    List<String> result = CoreUtil.parseList(
        conf.get(Config.STORM_ZOOKEEPER_SERVERS), Arrays.asList("localhost"));
    Assert.assertEquals(Arrays.asList("localhost"), result);
    conf.put(Config.STORM_ZOOKEEPER_SERVERS,
        Arrays.asList("server1", "server2"));
    result = CoreUtil.parseList(conf.get(Config.STORM_ZOOKEEPER_SERVERS),
  ...
  public void testParseInt() {
    ...
    int secs = Utils.getInt(defaultValue, 40);
    Assert.assertEquals(30, secs);
    conf.put(Config.TOPOLOGY_MESSAGE_TIMEOUT_SECS, null);
    secs = Utils.getInt(conf.get(Config.TOPOLOGY_MESSAGE_TIMEOUT_SECS), 40);
  ...
  public void testParseBoolean() {
    ...
    boolean flag = Utils.getBoolean(defaultValue, true);
    Assert.assertFalse(flag);
    conf.put(Config.TOPOLOGY_DEBUG, null);
    flag = Utils.getBoolean(conf.get(Config.TOPOLOGY_DEBUG), true);
  ...
  public void testPartitionFixed() {
    ...
    List<List<Integer>> ret = Utils.partitionFixed(5, aseq);
  ...
  public void testTokenize_path() {
    ...
    expected.add("xiong");
    expected.add("jstorm");
    Assert.assertEquals(expected, Zookeeper.tokenizePath(test));
  }
  ...
  public void testToks_to_path() {
    ...
    Assert.assertEquals(test, Zookeeper.toksToPath(expected));
  }
  ...
  public void testInterleaveAll() {
    ...
    Assert.assertTrue(
        SetUtils.isEqualSet(Arrays.asList(1, 8, 4, 2, 5, 3, 6, 7), result));
  ...
  public void testSetDifference() {
    ...
    NodeInfo ws1 = new NodeInfo("1162f9d6-90f5-4487-98fe-5f1693ec2b50", Sets.newHashSet(38977l));
    Set<NodeInfo> currentConnections = new HashSet<NodeInfo>();
    currentConnections.add(ws1);
    NodeInfo ws2 = new NodeInfo("1162f9d6-90f5-4487-98fe-5f1693ec2b50", Sets.newHashSet(38977l));
    Set<NodeInfo> neededConnections = new HashSet<NodeInfo>();

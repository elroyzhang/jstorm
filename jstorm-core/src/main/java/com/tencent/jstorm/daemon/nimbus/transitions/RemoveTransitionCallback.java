...
public class RemoveTransitionCallback extends BaseCallback {
  ...
  public <T> Object execute(T... args) {
    ...
    if (this.data.getBlobStore() instanceof LocalFsBlobStore) {
      List<String> blobKeys =
          NimbusUtils.getKeyListFromId(this.data.getConf(), stormId);
      for (String blobKey : blobKeys) {
        try {
          data.getStormClusterState().removeBlobstoreKey(blobKey);
          data.getStormClusterState().removeKeyVersion(blobKey);
        } catch (Exception e) {
          LOG.warn("Failed to remove StormBase " + stormId + " from ZK for {}",
              e);
        }
      }
    }

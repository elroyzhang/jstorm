...
public class ConfigUtils {
  ...
  public static String getLogDir() {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Illegal storm.log.dir in conf: " + dir);
    }
  }
  ...
  // ALL-CONFIGS is only used by executor.clj once, do we want to do it here?
  // TODO
  ...
  public static int samplingRate(Map conf) {
    ...
    throw new IllegalArgumentException(
        "Illegal topology.stats.sample.rate in conf: " + rate);
  }
  ...
  // public static mkStatsSampler // depends on Utils.evenSampler() TODO, this
  // is sth we need to do after util
  ...
  public static String stormDistPath(String stormRoot) {
    String ret = "";
    // we do this since to concat a null String will actually concat a "null",
  ...
  public static Map readSupervisorStormConfGivenPath(Map conf,
      String stormConfPath) throws IOException {
    Map ret = new HashMap(conf);
    ret.putAll(Utils.fromCompressedJsonConf(
        FileUtils.readFileToByteArray(new File(stormConfPath))));
    return ret;
  }
  ...
  public String supervisorStormDistRootImpl(Map conf, String stormId)
      throws IOException {
    return supervisorStormDistRoot(conf) + FILE_SEPARATOR
        + URLEncoder.encode(stormId, "UTF-8");
  }
  ...
  // we use this "weird" wrapper pattern temporarily for mocking in clojure test
  public static void setWorkerUserWSE(Map conf, String workerId, String user)
      throws IOException {
  ...
  public static String workerArtifactsPidPath(Map conf, String id,
      Integer port) {
  ...
  public static File getLogMetaDataFile(String fname) {
    String[] subStrings = fname.split(FILE_SEPARATOR); // TODO: does this work
                                                       // well on windows?
  ...
  public static File getLogMetaDataFile(Map conf, String id, Integer port) {
    String fname =
        workerArtifactsRoot(conf, id, port) + FILE_SEPARATOR + "worker.yaml";
    return new File(fname);
  }

  public static File getWorkerDirFromRoot(String logRoot, String id,
      Integer port) {
  ...
  public static Map overrideLoginConfigWithSystemProperty(Map conf) { // note
                                                                      // that we
                                                                      // delete
                                                                      // the
                                                                      // return
  ...
  public static List<String> getTopoLogsGroups(Map topologyConf) {
    List<String> logsGroups =
        (List<String>) topologyConf.get(Config.LOGS_GROUPS);
  ...
  @ClojureClass(className="for save dump files, http://git.code.oa.com/trc/jstorm/issues/29")
  public static String workerDumpsRoot(Map conf, String id, Integer port) {
    return workerArtifactsRoot(conf, id, port) + FILE_SEPARATOR + "dumps";
  }
}

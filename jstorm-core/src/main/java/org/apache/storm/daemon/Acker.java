...
public class Acker implements IBolt {
    ...
    public static final String ACKER_UPDATE_STREAM_ID = "__ack_update";
    ...
    private RotatingMap<Object, Integer> updatePending;
    ...
    @Override
    @ClojureClass(className="topology.bolts.update.enable false to disable update")
    public void prepare(Map stormConf, TopologyContext context, OutputCollector collector) {
        this.collector = collector;
        this.pending = new RotatingMap<Object, AckObject>(TIMEOUT_BUCKET_NUM);
        this.updatePending = new RotatingMap<Object, Integer>(TIMEOUT_BUCKET_NUM);
    ...
    public void execute(Tuple input) {
        if (TupleUtils.isTick(input)) {
            ...
            Map<Object, Integer> tmpUpdate = updatePending.rotate();
            LOG.debug("Number of timeout tuples:{}", tmp.size());
            LOG.debug("Number of timeout update tuples:{}", tmpUpdate.size());
            return;
        }
        ...
        /** Added by Jstorm developer */
        if (ACKER_UPDATE_STREAM_ID.equals(streamId)) {
          Integer spoutTask = (Integer) updatePending.remove(id);
          if (spoutTask != null) {
            collector.emitDirect(spoutTask, ACKER_UPDATE_STREAM_ID,
                new Values(input.getValue(1)));
          }
          return;
        }
        ...
        if (ACKER_INIT_STREAM_ID.equals(streamId)) {
            if (curr == null) {
                ...
                if(this.isUpdateEnable) {
                  updatePending.put(id, curr.spoutTask);
                }
            } else {

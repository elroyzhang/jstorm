...
public class SupervisorUtils {
    ...
    @ClojureClass(className="kill worker parallism")
    public static void shutdownAllWorkers(Map conf, String supervisorId, Map<String, String> workerThreadPids, Set<String> deadWorkers,
        ...
        Collection<String> workerIds = SupervisorUtils.supervisorWorkerIds(conf);
        try {
            workerManager.shutdownWorkers(supervisorId, new ArrayList<String>(workerIds), workerThreadPids);
            for(String workerId : workerIds){
    ...
    /**
     * added by JStorm developer , check version
     * 
     * @param supervisorData
     */
    public static void checkVersion(SupervisorData supervisorData) {
      List<NimbusSummary> nimbusSummaries =
          supervisorData.getStormClusterState().nimbuses();
      NimbusSummary nimbusLeader = null;
      for (NimbusSummary nimbusSummary : nimbusSummaries) {
        if (nimbusSummary.is_isLeader()) {
          nimbusLeader = nimbusSummary;
          break;
        }
      }
      if (nimbusLeader != null) {
        String supervisorVersion = VersionInfo.getVersion();
        if (!supervisorVersion.equals(nimbusLeader.get_version())) {
          throw new RuntimeException("supervisor version :" + supervisorVersion
              + " not match nimubus leader :" + nimbusLeader);
        }
      } else {
        throw new RuntimeException("nimbus leader still hasn't started");
      }
    }
}

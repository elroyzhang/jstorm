...
public class SetLogLevel {
  ...
  public static void main(String[] args) throws Exception {
    Map<String, Object> cl = CLI
        .opt("l", "log-setting", null,
            new LogLevelsParser(LogLevelAction.UPDATE), CLI.INTO_MAP)
        .opt("r", "remove-log-setting", null,
            new LogLevelsParser(LogLevelAction.REMOVE), CLI.INTO_MAP)
        .arg("topologyName", CLI.FIRST_WINS).parse(args);
      ...
      public void run(Nimbus.Client nimbus) throws Exception {
        ...
        if (null == topologyId) {
          throw new IllegalArgumentException(
              topologyName + " is not a running topology");
        }
        nimbus.setLogConfig(topologyId, logConfig);
        LOG.info("Log config {} is sent for topology {}", logConfig,
            topologyName);
      }
    });
  }
   ...
   *
   * e.g. ROOT=DEBUG:30 root logger, debug for 30 seconds
   *
   * org.apache.foo=WARN org.apache.foo set to WARN indefinitely
   */
  static final class LogLevelsParser implements CLI.Parse {
    ...
    @Override
    @ClojureClass(className="set target_log_level")

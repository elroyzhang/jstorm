...
public class NimbusBlobStore extends ClientBlobStore {
  ...
  @Override
  @ClojureClass(className = "try several times in case exception cause by update state on the same key in service handler")
    ...
    while (true) {
      try {
        ...
        if (retryTimes >= 2) {
          ...
          LOG.warn("createStateInZookeeper error , retry times {}: {}",
              retryTimes, e);
          retryTimes++;
        }
      }

...
public class DRPCSpout extends BaseRichSpout {
    ...
    @Override
    @ClojureClass(className="get drpc server ids from zk")
    public void open(Map conf, TopologyContext context, SpoutOutputCollector collector) {
        ...
        if(_local_drpc_id==null) {
            ...
            List<DRPCServerInfo> servers = DRPCClient.getDrpcServerInfosFromZK(conf);
            if(servers == null || servers.isEmpty()) {
                throw new RuntimeException("No DRPC servers configured for topology");   
            }
            
            if (numTasks < servers.size()) {
                for (DRPCServerInfo s: servers) {
                    _futures.add(_backround.submit(new Adder(s.getHost(), s.getInvocationPort(), conf)));
                }
            } else {        
                int i = index % servers.size();
                DRPCServerInfo s = servers.get(i);
                _futures.add(_backround.submit(new Adder(s.getHost(), s.getInvocationPort(), conf)));
            }
        }

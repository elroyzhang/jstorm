...
public class BoltExecutorStats extends CommonStats {
    ...
    @ClojureClass(className = "added by JStorm developer, long -> Long , fix bug NPE")
    ...
    @ClojureClass(className = "ack与process-latencies拆分计算,此处只计算ack数,屏蔽process-latencies累计内容")
    public void boltAckedTuple(String component, String stream, Long latencyMs) {
      List key = Lists.newArrayList(component, stream);
      this.getAcked().incBy(key, this.rate);
      // ack与process-latencies拆分计算,此处只计算ack数,屏蔽process-latencies累计内容
      // this.getProcessLatencies().record(key, latencyMs);
    }

    /**
     * Added by JStorm developer, Bolt process流程完成时间独立设置
     * 
     * @param stats
     * @param stream
     * @param latencyMs
     */
    @ClojureClass(className = "ack与process-latencies拆分计算,此处只计算process-latencies累计内容")
    public void boltRecordProcessLatencies(String component, String stream,
        Long latencyMs) {
      List key = Lists.newArrayList(component, stream);
      this.getProcessLatencies().record(key, latencyMs);
    }

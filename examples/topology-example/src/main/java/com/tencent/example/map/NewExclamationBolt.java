...
public class NewExclamationBolt extends BaseRichBolt {
  ...
  public void execute(Tuple tuple) {
    if (TupleUtils.isTick(tuple)) {
      ...
      if (tuple.getSourceStreamId().equals("STRAEM_ID_WORD")) {
        _collector.emit("STRAEM_ID_WORD", tuple,
            new Values(tuple.getString(0) + "!!!"));
      } else {
        _collector.emit("STRAEM_ID_COUNT", tuple,
            new Values(tuple.getString(0) + "!!!"));
      }
  ...
  public void declareOutputFields(OutputFieldsDeclarer declarer) {
    ...
    declarer.declareStream("STRAEM_ID_COUNT", new Fields("exclam"));
  }

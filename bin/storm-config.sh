...
if [ -f "${STORM_CONF_DIR}/storm-env.sh" ]; then
  chmod a+x ${STORM_CONF_DIR}/storm-env.sh
  . "${STORM_CONF_DIR}/storm-env.sh"
fi

#LOG4J_CONF="${STORM_CONF_DIR}/storm.log4j.properties"
LOG_CONTEXT_SELECTOR="org.apache.logging.log4j.core.async.AsyncLoggerContextSelector"
LOG4J_CONF_FILE="${STORM_CONF_DIR}/log4j2/cluster.xml"
cygwin=false
case "`uname`" in
CYGWIN*) cygwin=true;;
esac
...
#for f in $STORM_HOME/jstorm-{core,common}-*.jar; do
#  CLASSPATH=${CLASSPATH}:$f;
#done
...
for f in $STORM_HOME/tools/tdbank-sender/*.jar; do
  CLASSPATH=${CLASSPATH}:$f;
done
...
STORM_OPTS="$STORM_OPTS -Dstorm.conf.file=$STORM_CONF_DIR/storm.yaml"
#STORM_OPTS="$STORM_OPTS -Dlog4j.configuration=File:${LOG4J_CONF}"
STORM_OPTS="$STORM_OPTS -Dlog4j.configurationFile=${LOG4J_CONF_FILE}"
STORM_OPTS="$STORM_OPTS -DLog4jContextSelector=${LOG_CONTEXT_SELECTOR}"
STORM_OPTS="$STORM_OPTS -Dstorm.root.logger=${STORM_ROOT_LOGGER:-INFO,DRFA,STDERR}"
if [ "x$JAVA_LIBRARY_PATH" != "x" ]; then

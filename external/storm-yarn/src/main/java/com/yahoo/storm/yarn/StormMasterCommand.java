...
class StormMasterCommand implements ClientCommand {
  ...
  enum COMMAND {
    GET_STORM_CONFIG, SET_STORM_CONFIG, START_NIMBUS, STOP_NIMBUS, START_UI, STOP_UI, ADD_SUPERVISORS, START_SUPERVISORS, STOP_SUPERVISORS, SHUTDOWN, KILL_APPLICATION
  };
  ...
  public Options getOpts() {
    ...
    opts.addOption("supervisors", true,
        "(Required for addSupervisors) The # of supervisors to be added");
    return opts;
  }
  ...
  public void process(CommandLine cl,
    ...
    String stormZookeeperRoot =
        Util.parseString(
            stormConf.get(backtype.storm.Config.STORM_ZOOKEEPER_ROOT), "/storm")
            + "/" + appId;
    stormConf.put(backtype.storm.Config.STORM_ZOOKEEPER_ROOT,
        stormZookeeperRoot);

    LOG.info("set storm.zookeeper.root to appID: " + stormZookeeperRoot);
  ...
  public static void downloadStormYaml(StormMaster.Client client,
      String storm_yaml_output) {

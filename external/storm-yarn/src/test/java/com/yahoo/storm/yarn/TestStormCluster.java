...
public class TestStormCluster {
  ...
  public static void setup() throws InterruptedException, IOException {
    ...
    final Map storm_conf = Config
        .readStormConfig("src/main/resources/master_defaults.yaml");
    ...
    storm_conf.put(backtype.storm.Config.UI_PORT,
        Util.assignServerSocketPort(-1));
  ...
  private static void checkNimbusConnection(
      @SuppressWarnings("rawtypes") final Map storm_conf) throws IOException,
      UnknownHostException {
    // Try to open a TCP connection to the nimbus port.
    new Socket((String) storm_conf.get(Config.MASTER_HOST),
        (Integer) storm_conf.get(backtype.storm.Config.NIMBUS_THRIFT_PORT))
        .close();
  }
  ...
  private static void checkZkConnection(
    ...
    new Socket("localhost",
        (Integer) storm_conf.get(backtype.storm.Config.STORM_ZOOKEEPER_PORT))
        .close();
  }
  ...
  private static void checkUiConnection(Map<?, ?> storm_conf)
    ...
    new Socket((String) storm_conf.get(Config.MASTER_HOST),
        (Integer) storm_conf.get(backtype.storm.Config.UI_PORT)).close();

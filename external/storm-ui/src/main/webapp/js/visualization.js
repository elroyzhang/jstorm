  ...
  var myRenderer = {
    ...
    redraw : function() {
      ...
      psys.eachEdge(function(edge, pt1, pt2) {
        ...
        ctx
            .lineTo(newpt2.x - arrlen * Math.cos(theta - Math.PI / 6), newpt2.y - arrlen
                * Math.sin(theta - Math.PI / 6));
      ...
      psys.eachNode(function(node, pt) {
        ...
        // var real_trans = gather_stream_count(node.data[":stats"], "default",
        // "600");
        ...
        var w = Math.max(75, 35 + gfx.textWidth(node.name));
        ...
        gfx.text(parseFloat(node.data[":latency"]).toFixed(2) + " ms", pt.x, pt.y + 17, {
          color : "white",
          align : "center",
          font : "Arial",
          size : 12
...
function calculate_texts(psys, totaltrans) {
  ...
  psys.eachEdge(function(edge, pt1, pt2) {
    ...
    for (var i = 0; i < edge.target.data[":inputs"].length; i++) {
      ...
      if (stream_checked(sani_stream) && edge.target.data[":inputs"][i][":component"] === edge.source.name) {
        stream_transfered = gather_stream_count(edge.source.data[":stats"], sani_stream, "600");
        text += stream + ": " + stream_transfered + ": "
      ...
      if (stream_checked(node.target.data[":inputs"][i][":sani-stream"])) {
        if (!countedmap[node.source.name][stream]) {
          if (node.source.data[":stats"]) {
            var toadd = gather_stream_count(node.source.data[":stats"], node.target.data[":inputs"][i][":sani-stream"],
                "600");
            totaltrans += toadd;
          }
...
function has_checked_stream_output(jdat, component) {
  ...
  $.each(jdat, function(k, v) {
    for (var i = 0; i < v[":inputs"].length; i++) {
      if (stream_checked(v[":inputs"][i][":sani-stream"]) && v[":inputs"][i][":component"] == component)
        ret = true;
    }
  });
  return ret;
  ...
  if (stats)
    for (var i = 0; i < stats.length; i++) {
      if (stats[i][":transferred"] != null) {
        ...
        var tranData = stats[i][":transferred"];
        for (var j = 0; j < tranData.length; j++) {
          for ( var tmp in tranData[j]) {
            if (tmp == time) {
              for (var k = 0; k < tranData[j][tmp].length; k++) {
                for ( var tmpV in tranData[j][tmp][k]) {
                  if (tmpV == stream) {
                    transferred += tranData[j][tmp][k][stream];
                  }
                }
              }
            }
...
function rechoose(jdat, sys, box) {
  var id = box.id;
  if ($(box).is(':checked')) {
    // Check each node in our json data to see if it has inputs from or outputs
    // to selected streams. If it does, add a node for it.
    $.each(jdat, function(k, v) {
    ...
    // Check each edge to see if it represents any selected streams. If not,
    // prune it.
    sys.eachEdge(function(edge, pt1, pt2) {
      ...
      if ($.grep(inputs, function(input) {
        ...
        return input[":component"] === edge.source.name && stream_checked(input[":sani-stream"]);
  ...
  // Tell the particle system's renderer that it needs to update its labels,
  // colors, widths, etc.
...
function jsError(other) {
  try {
    ...
    $.get(proxyPath() + "templates/json-error-template.html", function(template) {
...
function show_visualization(sys) {
  $.getJSON(proxyPath() + "api/v1/topology/visualization-init?tid=" + $.url("?id"), function(response, status, jqXHR) {
    $.get(proxyPath() + "templates/topology-page-template.html", function(template) {
      jsError(function() {
        var topologyVisualization = $("#visualization-container");
        topologyVisualization.append(Mustache.render($(template).filter("#topology-visualization-container-template")
            .html(), response));
      });
      ...
      var update = function(should_rechoose) {
        if (should_update) {
          $.ajax({
            url : proxyPath() + "api/v1/topology/visualization?tid=" + $.url("?id"),
            type : "GET",
            dataType : "json",
            async : false,
            success : function(data, status, jqXHR) {
              ...
              if (data != null && data.length > 0) {
                for (tmp in data) {
                  var cid = data[tmp][":component"];
                  tmpData[cid] = data[tmp];
                }
              }
              topology_data = tmpData;

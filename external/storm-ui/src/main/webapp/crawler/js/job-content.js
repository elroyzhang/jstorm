...
function setJobName(siteId) {
  $("#divParam").find("#topoName").val("tdspider-job-" + siteId);
  $("#divParam").find("#topoName").prop("disabled", "disabled");
}
function setJobConfigVal(siteId) {
  if (siteId != "") {
    $("#divParam").find("#jonbConfig").val(siteId + ".yaml");
    $("#divParam").find("#jonbConfig").prop("disabled", "disabled");
  }
}
...
function getValues(arr, index) {
  ...
  for (var i = index; i < arr.length; i++) {
    ...
    if (i == index) {
      ...
      if (tmp.indexOf(":") > -1 && tmp.trim().indexOf("http:") == -1) {
        break;
      } else {
        tmp = tmp.trim();
        tmp = replaceWord(tmp);
...
function addParam() {
  var paramDivBody = $("#divParam").find("tbody");
  var topologyArgsTr = '<tr id="argsTr"><td style="width:20%"><label>topology args:</label></td>'
      + '<td style="width:40%"><input type="text" id="args" class="args"></input>'
  ...
  paramDivBody.append(topologyArgsTr);
}
...
function submitTopology() {
  ...
  var mainClass = $("#divParam").find("#mainClass").val();
  ...
  var submitUrl = proxyPath() + "api/v1/crawler/jobs?jar-type=tdspider.jar&jar-name=" + jarName + "&site.id=" + siteId
      + "&main-class=" + mainClass + "&args=" + args;
...
function assembleParams() {
  var argsStr = $("#divParam").find("#topoName").val() + "," + $("#divParam").find("#jonbConfig").val() + ",";
  $("#divParam").find("table").find("tbody").find("tr").each(function() {
    if ($(this).attr("id") == "argsTr") {
